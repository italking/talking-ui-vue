var Loading = {
    ...
    computed:{
        style:function(){
            return {backgroundColor:"#9d9d9d",opacity: 0.5,position: "fixed",left: 0,top: 0,zIndex:9999};
        },
        lp:function(){
            return {
                left:this.width / 2 - 125,
                top:this.height / 2 - 125
            }
        },
        lStyle:function(){
            return {
    ...
    //这个组件默认是不会销毁的，所以destroyed 不刷新浏览器不会调用，监控显示属性进行事件卸载
    watch:{
        visible:{
            handler(newV, oldV) {
                if(newV){
